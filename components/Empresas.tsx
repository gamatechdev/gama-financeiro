...
import {
    ...
    CheckCircle, AlertCircle, Building, Edit, Mail, Phone, Map, DollarSign, Calendar, User, Briefcase, FileText, Settings, LayoutGrid, List, ChevronLeft, ChevronRight, ChevronDown
} from 'lucide-react';
...
const Empresas: React.FC<EmpresasProps> = ({
    ...
    const [calendarYear, setCalendarYear] = useState(sharedDate.getFullYear());
                    ...
                    {filteredClientes.map(cliente => (
                        <div key={cliente.id} className="glass-panel p-6 rounded-[24px] relative group hover:bg-white/80 transition-all hover:translate-y-[-4px] border border-white/60">
                            ...
                            <div className="pt-4 border-t border-slate-100 flex flex-wrap gap-2">
                                ...
                                {cliente.responsavel && (
                                    <span className="px-2 py-1 rounded-lg bg-slate-50 text-slate-500 text-[10px] font-bold uppercase">
                                        {cliente.responsavel}
                                    </span>
                                )}
                            </div>
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-slate-50/50 border-b border-slate-100">
                                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Empresa</th>
                                    <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">CNPJ / Email</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {filteredClientes.map(cliente => (
                                    <tr key={cliente.id} className="hover:bg-white/60 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-400 line-clamp-1 max-w-[200px]">{cliente.razao_social}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <p className="text-sm text-slate-600 font-mono">{cliente.cnpj || '—'}</p>
                                            <p className="text-xs text-slate-400">{cliente.email || '—'}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex gap-2">
                                                {cliente.status && (
                                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${cliente.status === 'Ativo' ? 'bg-green-50 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
                                                        {cliente.status}
                                                    </span>
                                                )}
                                                {cliente.envia_esoc && (
                                                    <span className="px-2 py-0.5 rounded-full bg-teal-50 text-[#149890] text-[10px] font-bold uppercase">eSocial</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button
                                                    onClick={() => handleEdit(cliente)}
                                                    className="p-2 rounded-lg bg-slate-50 text-slate-400 hover:text-[#04a7bd] hover:bg-cyan-50 transition-colors"
                                                >
                                                    <Edit size={16} />
                                                </button>
                                                <button
                    ...
                    <div className="glass-panel w-full max-w-6xl rounded-[32px] relative z-10 p-0 overflow-hidden bg-white/95 shadow-2xl border border-white/60 animate-[scaleIn_0.2s_ease-out] flex flex-col max-h-[95vh]">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                ...
                                <div className="lg:col-span-4 bg-slate-50/50 rounded-3xl p-6 border border-slate-100 flex flex-col h-full max-h-[600px] lg:max-h-full">
