...
const Despesas: React.FC = () => {
  ...
  const filterByMonth = (d: FinanceiroDespesa) => {
    ...
    // Logic: 
    // 1. Exact month match
    // 2. OR Recurring is true AND start date is before or equal to selected month
  ...
  const filteredDespesas = useMemo(() => {
    return despesas.filter(d => {
      // Month & Recurring Logic
      if (!filterByMonth(d)) return false;

      // Search (Name or Fornecedor)
  ...
  return (
    <div className="p-6 relative min-h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-800">Despesas</h2>
                  ...
                  <div className="flex items-center gap-2">
                     {!isPaid && (
                        <button 
                          ...
                          className="group h-10 bg-green-500 hover:bg-green-600 text-white rounded-full flex items-center transition-all duration-300 shadow-lg shadow-green-500/30 overflow-hidden w-10 hover:w-[160px]"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="glass-panel w-full max-w-lg rounded-[32px] relative z-10 p-8 animate-[scaleIn_0.2s_ease-out] bg-white/80 shadow-2xl border border-white/60 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-2xl font-bold text-slate-800">{editingId ? 'Editar Despesa' : 'Nova Despesa'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-slate-100/50 p-1.5 rounded-2xl flex relative">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${formData.responsavel === 'Gama Medicina' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Gama Medicina
                </button>
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${formData.responsavel === 'Gama Soluções' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Gama Soluções
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div className="space-y-1">
                    ...
                    <input 
                      ...
                      placeholder="1"
                      disabled={formData.recorrente} // Disable parcels if recurring is checked
                    />
                 </div>
              </div>
