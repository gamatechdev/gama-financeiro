...
const Despesas: React.FC = () => {
  ...
  const kpiDespesas = useMemo(() => {
    // FIX #6: KPIs não devem contar despesas vencidas de meses passados
    return despesas.filter(d => !(d as any)._isOverdueFromPast);
  }, [despesas]);
  ...
  const { specialProviderCards, paymentGroupCards, regularDespesas } = useMemo(() => {
    ...
    filteredDespesas.forEach(d => {
      ...
      if (d.nome && d.nome.toUpperCase() === "PAGAMENTO") {
        ...
        // FIX #7: recalcula status baseado na contagem de IDs
        const pt = paymentMap[providerKey];
        const hasPending = pt.pendingIds.length > 0;
        const hasPaid = pt.count > pt.pendingIds.length;
