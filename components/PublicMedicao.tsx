...
import {
    ...
    QrCode, Copy, Barcode, Download, Smartphone, CreditCard, Stethoscope, ArrowRight, LayoutList, Eye, FileText, Search
} from 'lucide-react';
...
const PublicMedicao: React.FC<PublicMedicaoProps> = ({ dataToken }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const { clientId, month, startDate, endDate, label } = JSON.parse(decoded);

                if (!clientId) throw new Error("Link inválido");

                setRangeLabel(label || '');

                // Use dates from token if available, otherwise fallback to month calc
                let start = startDate;
                let end = endDate;

                if (!start || !end) {
                    if (!month) throw new Error("Link incompleto");
                    const [y, m] = month.split('-').map(Number);
                    start = new Date(y, m - 1, 1).toISOString().split('T')[0];
                    end = new Date(y, m, 0).toISOString().split('T')[0];

                    // Backup range label if none provided
                    if (!label) {
                        const dateObj = new Date(y, m - 1, 15);
                        const monthName = dateObj.toLocaleString('pt-BR', { month: 'long' });
                        setRangeLabel(`${monthName.charAt(0).toUpperCase() + monthName.slice(1)} de ${y}`);
                    }
                }
    ...
    const examReceitas = useMemo(() => {
        return receitas
            .filter(r => r.exames_snapshot && Array.isArray(r.exames_snapshot) && r.exames_snapshot.length > 0)
            .filter(r => (r.descricao || '').toLowerCase().includes(examSearchTerm.toLowerCase()));
    }, [receitas, examSearchTerm]);
                ...
                <div className="lg:col-span-4 space-y-4 print:w-full">
                    ...
                    <div className="relative mb-4">
                        ...
                        <input
                            ...
                            className="w-full bg-white border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-xs font-medium focus:outline-none focus:border-blue-400 transition-colors shadow-sm"
                        />
                    </div>
                    ...
                    <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1 custom-scrollbar">
                        {examReceitas.length === 0 ? (
                            <div className="p-6 text-center text-slate-400 bg-white rounded-2xl border border-dashed border-slate-200">
                                {examSearchTerm ? 'Nenhum atendimento encontrado para esta busca.' : 'Nenhum atendimento médico listado.'}
                            </div>
                        ) : (
